...
var Selectize = function ($input, settings) {
	...
	dir = window.getComputedStyle ? window.getComputedStyle(input,
	                                                        null).getPropertyValue('direction') : input.currentStyle
		      && input.currentStyle.direction;
...
$.extend(Selectize.prototype, {
	...
	setup: function () {
		...
		$control_input = $('<input type="search" autocomplete="off" class="search"/>').appendTo($control).attr('tabindex',
		                                                                                                       tab_index);
		...
		$dropdown.on('mouseenter',
		             '[data-selectable]',
		             function () { return self.onOptionHover.apply(self, arguments); });
		$dropdown.on('mousedown',
		             '[data-selectable]',
		             function () { return self.onOptionSelect.apply(self, arguments); });
		watchChildEvent($control,
		                'mousedown',
		                '*:not(input)',
		                function () { return self.onItemSelect.apply(self, arguments); });
	 ...
	 */
	showInput: function () {
		this.$control_input.css({opacity: 1, position: ''});
	...
	search: function (query) {
		...
		if (settings.score) {
			calculateScore = self.settings.score.apply(this,
			                                           [
				                                           query
			                                           ]);
		...
		try {
			...
			for (i = 0; i < n; i++) {
				...
				if (option) {
					...
					optgroups = $.isArray(optgroup) ? optgroup :
					            [
						            optgroup
					            ];
			...
			if (self.hasOptions) {
				...
				$dropdown_content[0].scrollTop = 0;
			} else {
				self.setActiveOption(null);
	 ...
	 * @param {string} value
	 */
	removeOption: function (value, simple) {
		var self = this;
	...
	addItem: function (value) {
		debounce_events(this,
				...
				if (self.isSetup && !self.isPending) {
					...
					if (!options.length ||
					    (self.settings.maxItems !== null && self.items.length >= self.settings.maxItems)) {
						self.close();
					} else {
	...
	refreshClasses: function () {
		...
		self.$control.toggleClass('focus', self.isFocused).toggleClass('disabled',
		                                                               self.isDisabled).toggleClass('required',
		                                                                                            self.isRequired).toggleClass('invalid',
		                                                                                                                         self.isInvalid).toggleClass('locked',
		                                                                                                                                                     isLocked).toggleClass('full',
		                                                                                                                                                                           isFull).toggleClass('not-full',
		                                                                                                                                                                                               !isFull).toggleClass('input-active',
		                                                                                                                                                                                                                    self.isFocused
			                                                                                                                                                                                                                    && !self.isInputHidden).toggleClass('dropdown-active',
		                                                                                                                                                                                                                                                            self.isOpen).toggleClass('has-options',
		                                                                                                                                                                                                                                                                                     !$.isEmptyObject(self.options)).toggleClass('has-items',
		                                                                                                                                                                                                                                                                                                                                 self.items.length
			                                                                                                                                                                                                                                                                                                                                 > 0);
	...
	updateOriginalInput: function () {
		...
		if (self.$input[0].tagName.toLowerCase() === 'select') {
			...
			self.$input[0].innerHTML = options.join('');
		} else {
	...
	insertAtCaret: function ($el) {
		...
		if (caret === 0) {
			this.$control.append($el);
		} else {
			$(this.$control[0].childNodes[caret]).after($el);
		}
	...
	deleteSelection: function (e) {
		...
		if (!values.length || (typeof self.settings.onDelete
			                       === 'function'
			&& self.settings.onDelete.apply(self,
			                                [
				                                values
			                                ])
			                       === false)) {
	...
	destroy: function () {
		...
		self.$input[0].innerHTML = '';
		self.$input.append(revertSettings.$children).removeAttr('tabindex').attr({tabindex: revertSettings.tabindex}).show();
...
Selectize.define('optgroup_columns', function () {
	var self = this;
	...
	var equalizeSizes = function () {
		...
		// Use CSS to handle width stuff for us but
		_($optgroups).each(function (opt) {

			opt.className =
			[
				'optgroup',
				'count-' + $optgroups.length,
				opt.getAttribute('data-group').toLowerCase().replace(/\s/g, '')
			].join(' ');
